 ...
 *
 *
 * Some modifications by Hauke Gravenkamp 2024*/
...
function BibtexDisplay() {
    ...
    this.displayAuthor = function (string, format) {
        ...
        var maxAuthors = 6; // Set the limit of authors before 'et al.' is added

        // Get the max amount of authors to print
        var searchLength = Math.min(maxAuthors, arrayString.length); // Limit to 6 authors
        ...
        for (i = 0; i < searchLength; i++) {
            ...
            // Abbreviate the first name (assuming first name is index 0)
            var firstName = name[0].trim(); // Get first name
            var abbreviatedFirstName = firstName.charAt(0).toUpperCase() + "."; // Abbreviate
            name[0] = abbreviatedFirstName; // Replace first name with abbreviation
        ...
        if (searchLength != arrayString.length) {
            ...
            newString += " <span class='etal'>et al.</span>";  // Wrap 'et al.' in a span with class 'etal'
        }
    ...
    this.createTemplate = function (entry, output) {
        ...
        for (var index in keys) {
            ...
            if (key == "AUTHOR") {
                ...
                var doiUrl = "https://doi.org/" + value; // Create the full DOI URL
                value = '<a href=' + doiUrl + ' target="_blank";>' + value + '</a>';
            } else {
                value = this.fixValue(value);
            }
        ...
        if (groupChild.length) {
            ...
            for (var entryKey in entries) {
                ...
                if (!entry["WEB"] || entry["WEB"].toUpperCase() != "NO") {
                    ...
                    if (tpl) {
                        // Create a wrapper for the entire publication entry
                        var entryWrapper = $("<div></div>")
                            .addClass("publication-entry")  // Add the class for CSS
                            .css("display", "block");  // Make sure it's a block-level element

                        // Add publication number inline
                        var numberElement = $("<span></span>")
                            .addClass("publication-number")
                            .text("[" + publicationNumber + "] ");

                        // Prepend the number to the publication entry
                        tpl.css("display", "inline").prepend(numberElement);

                        // Append the entry inside the wrapper
                        entryWrapper.append(tpl);

                        // Append the wrapper to the structure
                        structure.find(".templates").append(entryWrapper);
...
function bibtex_js_draw() {
    ...
    $(".first").each(function () {
        var firstName = $(this).text().trim();
        if (firstName) {
            var abbreviatedName = firstName.charAt(0).toUpperCase() + "."; // Abbreviate the first name
            $(this).text(abbreviatedName); // Replace the first name with the abbreviated version
        }
    });
